...
/*
* The base class for the twitter feed for developers.
*/
...
class Twitter {
  ...
  function getTweets($screenname = false, $count = 20, $options = false ) {
    ...
    if (is_int($screenname)) {
      list($screenname, $count) = [ $count, $screenname ];
    }
    ...
    if (false === $screenname || 20 === $screenname) { $screenname = $this->defaults['screenname'];
    }
    ...
    if (is_array($result) && isset($result['errors'])) {
      ...
      return [
      'error' => 'Twitter said: ' . wp_json_encode($last_error),
      ];
    } else {
      if (is_array($result)) {
        return $this->cropTweets($result,$count);
      } else {
        $last_error = 'Something went wrong with the twitter request: ' . wp_json_encode($result);
        return [
        'error' => $last_error,
        ];
      }
  ...
  private function oauthGetTweets($screenname, $options ) {
    ...
    if (empty($secret)) { return [
      'error' => 'Missing Consumer Secret - Check Settings',
    ];
    }
    ...
    if (empty($token_secret)) { return [
      'error' => 'Missing Access Token Secret - Check Settings',
    ];
    }
    if (empty($screenname)) { return [
      'error' => 'Missing Twitter Feed Screen Name - Check Settings',
    ];
    }
    ...
    if (! isset($result['errors'])) {
      ...
      $file = $this->getCacheLocation();
      file_put_contents($file, wp_json_encode($cache));
    } else {
